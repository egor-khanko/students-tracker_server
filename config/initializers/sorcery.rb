...
Rails.application.config.sorcery.submodules = %i[remember_me reset_password brute_force_protection activity_logging external]
...
Rails.application.config.sorcery.configure do |config|
  ...
  #
  # config.vk.key = Rails.application.credentials.vk[:client_id]
  # config.vk.secret = Rails.application.credentials.vk[:secret]
  # config.vk.callback_url = 'http://localhost:3000/oauth/callback?provider=vk'
  # config.vk.user_info_mapping = { login: 'domain', name: 'full_name' }
  # config.vk.api_version = '5.131'
  #
